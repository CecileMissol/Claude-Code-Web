# Etsy listing — "Riviera Postcard" wedding theme (`mariage-riviera-postcard`)
...
> **Riviera Postcard Wedding Website Template No Canva | Animated Digital Invitation Envelope Opening RSVP | Mediterranean Save the Date**

132 characters.

## 13 tags (≤ 20 characters each)

1. `riviera wedding` (16)
2. `wedding website` (15)
3. `save the date` (13)
4. `digital invitation` (18)
5. `animated invite` (15)
6. `postcard wedding` (17)
7. `no canva needed` (15)
8. `wedding rsvp site` (17)
9. `coastal wedding site` (20)
10. `editable website` (16)
11. `wedding invite link` (19)
12. `lemon wedding invite` (20)
...
"Style: Coastal, Mediterranean, Destination". List as a digital download,
not under physical paper goods.
...
Your guests tap a cobalt-blue envelope stamped with a sun-shaped postmark —
it flips, opens, and your save-the-date ticket and photos slide out onto
scalloped postcard edges and citrus-bright colors. Then they scroll through
your story, your date, your program and your venue, laid out like a
Mediterranean travel postcard: sun-drenched photos with deckled white
borders, lemon-branch sprigs, striped beach-cabana ribbon, and cobalt,
citron and terracotta accents.
...
- Access to the {{BRAND_NAME}} guided editor (no design software, no Canva
  account)
- The "Riviera Postcard" animated wedding website theme, with 3 color
  palettes (Côte d'Azur, Positano Sunset, Capri Citrus) and 3 signature
  scripts
...
real phones. Colors — including on illustrations like the envelope, sun
postmark and stamps — are genuinely customizable, not fixed images.
...
Overlay text set in Bodoni Moda (titles) / Outfit (labels), on blanc cassé
`#FDF8F0` or cobalt `#1E3A5F` per
`src/themes/mariage-riviera-postcard/manifest.ts` palette intent.
