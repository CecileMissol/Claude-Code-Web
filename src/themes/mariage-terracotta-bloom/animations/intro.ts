 ...
 *
 * One GSAP timeline, paused, replayable: flip → the wax seal cracks off → the
 * flap swings open → the "Save the date" ticket and the two polaroids rise out
 * → flowers tuck themselves into the composition → scrolling is released.
 *
 * The beats are the mock-up's, to the millisecond (see `INTRO`). GSAP replaces
 * the mock-up's `setTimeout` chain because a timeline can be killed, scrubbed,
...
export interface IntroElements {
  ...
  flap: Element | null;
  ticket: Element | null;
  polaroid1: Element | null;
  polaroid2: Element | null;
  tuck1: Element | null;
  tuck2: Element | null;
}
...
export function resetIntro(gsap: Gsap, els: IntroElements): void {
  ...
  set(els.ticket, { yPercent: 0, rotate: 0 });
  set(els.polaroid1, { xPercent: 0, yPercent: 0, rotate: 0 });
  set(els.polaroid2, { xPercent: 0, yPercent: 0, rotate: 0 });
