...
import type { Messages } from '@/themes/mariage-riviera-postcard/messages';
import {
  buildSubmission,
  clampGuests,
  errorMessage,
  isHoneypotFilled,
  shouldCallApi,
} from '@/themes/mariage-riviera-postcard/sections/rsvp-logic';
...
function renderRsvp(rsvp: Partial<RsvpSettings> = {}, mode: InvitationMode = 'demo'): string {
  return renderToStaticMarkup(
    createElement(Invitation, {
      ...
      mode,
      slug: mode === 'public' ? 'nora-and-luca' : undefined,
    }),
  );
}
...
describe('shouldCallApi', () => {
  it('only lets a published invitation write a reply', () => {
    expect(shouldCallApi('public', 'nora-and-luca')).toBe(true);
  });

  it('never writes from the editor preview or the public demo', () => {
    expect(shouldCallApi('preview', 'nora-and-luca')).toBe(false);
    expect(shouldCallApi('demo', 'nora-and-luca')).toBe(false);
  });
...
describe('the rendered form', () => {
  ...
  it('carries the postal seal that lands on the card once the reply is in', () => {
    const html = renderRsvp();
