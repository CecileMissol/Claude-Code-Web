...
import type { Messages } from '@/themes/mariage-noir-ivoire/messages';
import {
  buildSubmission,
  clampGuests,
  errorMessage,
  isHoneypotFilled,
  shouldCallApi,
} from '@/themes/mariage-noir-ivoire/sections/rsvp-logic';
...
function renderRsvp(rsvp: Partial<RsvpSettings> = {}, mode: InvitationMode = 'demo'): string {
  return renderToStaticMarkup(
    createElement(Invitation, {
      ...
      mode,
      slug: mode === 'public' ? 'zoe-et-dylan' : undefined,
    }),
  );
}
...
describe('shouldCallApi', () => {
  it('only lets a published invitation write a reply', () => {
    expect(shouldCallApi('public', 'zoe-et-dylan')).toBe(true);
  });

  it('never writes from the editor preview or the public demo', () => {
    expect(shouldCallApi('preview', 'zoe-et-dylan')).toBe(false);
    expect(shouldCallApi('demo', 'zoe-et-dylan')).toBe(false);
  });
...
describe('the rendered form', () => {
  ...
  it('always carries the hidden honeypot, out of reach of keyboard and screen readers', () => {
    ...
    expect(html).toContain('class="trap"');
    expect(html).toContain('tabindex="-1"');
  });

  it('shows the data-protection notice below the form, in the invitation language', () => {
    expect(renderRsvp()).toContain('supprimée six mois après');
    const english = renderToStaticMarkup(
      createElement(Invitation, { content: { ...content, locale: 'en' as const }, mode: 'demo' }),
    );
    expect(english).toContain('deleted six months after the event');
  });
  ...
  it('replaces the form with a closing notice when RSVP is disabled', () => {
    ...
    expect(html).toContain('Les réponses sont closes.');
  });
  ...
  it('carries the wax seal that lands on the card once the reply is in', () => {
    const html = renderRsvp();
    expect(html).toContain('class="done-seal"');
    expect(html).toContain('class="thanks"');
  });
});
