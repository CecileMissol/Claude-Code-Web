...
import { formatTime } from '@/themes/mariage-riviera-postcard/animations/format';

/**
 * `content.event.time` and `ProgramItem.time` are stored as `HH:MM`. That is a
 * storage format, not a reading format: a French invitation prints "14h30", an
 * English one "2:30 pm". The menu board of this theme prints nothing else.
 */
describe('formatTime', () => {
