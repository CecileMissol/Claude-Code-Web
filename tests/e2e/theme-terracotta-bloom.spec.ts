...
/**
 * Visual acceptance of the "Terracotta Bloom" theme (bohemian desert).
 *
 * The screenshots in `tests/e2e/screenshots/terracotta-bloom/` are the
 * deliverable: they are what gets reviewed after every change to the theme, on
 * the three viewports of the brief (390 / 768 / 1440 px).
 */

const THEME = 'mariage-terracotta-bloom';
const SHOTS = 'tests/e2e/screenshots/terracotta-bloom';
...
test.describe('Terracotta Bloom', () => {
  test('opens, scrolls through every chapter and is captured', async ({ page }, testInfo) => {
    ...
    await expect(invitation).toHaveAttribute('data-palette', 'sienna');
    ...
    await expect(page.locator('.tb-hint')).toHaveText('Tap the envelope to break the seal');
    ...
    // The arch card and the snaps are out; the scroll cue is showing.
    await expect(page.locator('.tb-card .tb-c1')).toHaveText('Save the date');
    await expect(page.locator('.tb-cue')).toContainText('keep scrolling');
    ...
    await expect(page.getByRole('heading', { name: 'How we began' })).toBeVisible();
    await expect(page.locator('[data-chapter="story"] .tb-note')).toHaveText('she said yes');
    await expect(page.locator('[data-chapter="story"] .tb-barcard-title')).toHaveText(
      'The Cactus Bar',
    );
    ...
    await expect(page.locator('[data-chapter="date"] .tb-bigscript')).toHaveText('the big day');
    await expect(page.getByRole('timer')).toBeVisible();
    // The countdown is running, not stuck on the server's zeros.
    const days = await page.locator('.tb-count b').first().textContent();
    ...
    await expect(page.locator('[data-chapter="place"] .tb-postcard .tb-img span')).toHaveText(
      'Joshua Tree',
    );
    ...
    await expect(page.locator('.tb-tag.on').first()).toBeVisible();
    ...
    await page.getByLabel('Full name').fill('Alice Mendez');
    await page.getByLabel('Email').fill('alice@example.com');
    await page.getByText('Count us in').click();
    await page.getByRole('button', { name: 'Send my reply' }).click();

    // Demo mode fakes the success: the wax seal lands and the thanks show.
    await expect(page.locator('.tb-rsvp.tb-sent')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('.tb-thanks')).toHaveText('Thank you — your reply has landed.');
  ...
  test('respects prefers-reduced-motion', async ({ page }, testInfo) => {
    ...
    await page.getByRole('heading', { name: 'How we began' }).scrollIntoViewIfNeeded();
    await page.waitForTimeout(500);
    const hidden = await page.evaluate(
      () =>
        [...document.querySelectorAll('[data-chapter="story"] .tb-line')].filter(
...
function countPieces(page: Page, chapter: string) {
  return page.locator(`[data-chapter="${chapter}"] .tb-board [data-at]`).count();
}

/** Number of those pieces that have arrived. */
function countOn(page: Page, chapter: string) {
  return page.locator(`[data-chapter="${chapter}"] .tb-board [data-at].on`).count();
}
